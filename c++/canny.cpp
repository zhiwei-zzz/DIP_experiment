...
int MyCanny(const Mat gray_image, Mat& fin_image, int high_threshold, int low_threshold)
{
   ...
   // imwrite("high_threshold_image.jpg", high_threshold_image);
    ...
    // 边缘连接
    fin_image = Mat::zeros(gray_image.rows, gray_image.cols, CV_8UC1);
    EdgeTracking(high_threshold_image, low_threshold_image, fin_image);
   // imshow("edge_tracking_image", fin_image);
    //imwrite("edge_tracking_image.jpg", fin_image);
